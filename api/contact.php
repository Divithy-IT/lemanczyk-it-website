...
function respond(int $status, bool $ok, string $code, string $message, int $retryAfter = 0): never {
    http_response_code($status); if ($retryAfter > 0) header('Retry-After: ' . $retryAfter);
    echo json_encode(['ok' => $ok, 'code' => $code, 'message' => $message], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); exit;
}
...
try {
    ...
    respond($error->status, false, $error->publicCode, $error->getMessage(), $error->retryAfter);
